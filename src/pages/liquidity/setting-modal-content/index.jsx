...
export default function SettingModalContent() {
	...
	const [speedSelectedItem, setSpeedSelectedItem] = useState(speedListTabs.at(0));
	...
	const [slipSelectedItem, setSlipSelectedItem] = useState(slipListTabs.at(0));
	const slipTabChangeHandle = (item) => setSlipSelectedItem(item);
	const renderClassShowWarning = () => slipSelectedItem.value === slipListTabs.at(0).value ? '' : 'd-0'
							...
							content={
								<>
									<p className='mb-4'>
										...
										transaction. Higher GWEL = higher speed = higher
										fees.
									</p>
									<p>
										Choose "Default" to use the settings from your
										current blockchain RPC node
									</p>
								</>
							}
						>
				...
				<div className={css.settingModalContent__controls}>
					<div className={css.settingModalContent__title}>
						...
						<ToolTip
							className={`inline-flex items-center`}
							content={
								`Settings a high slippage tolerance can help transaction succeed, but you may not get such a good price. Use with caution.`
							}
						>
							<FaRegQuestionCircle />
						</ToolTip>
					...
					<div className={`${renderClassShowWarning()} ${css.settingModalContent__warning}`}>
						Your transaction may fail
					</div>
				</div>
				<div className={`${css.settingModalContent__controls} ${css.row}`}>
					<div className={css.settingModalContent__title}>
						Tx deadline (mins)
						<ToolTip
							className={`inline-flex items-center`}
				...
				<div className={`${css.settingModalContent__controls} ${css.row}`}>
					<div className={css.settingModalContent__title}>
						...
						<ToolTip
							className={`inline-flex items-center`}
							content={
								`Increase the speed of finding best swapping routes`
							}
						>
							<FaRegQuestionCircle />
						</ToolTip>
