...
function SectionInfo(props) {
	...
	const renderInfoRow = (periodsList) => {
		return periodsList.map(([item1, item2, item3], index) => (
			<div key={index} className={`${css.sectionInfo}`}>
	...
	return (
		<div
			className={`${renderHideInfo()} ${css.sectionInfo__container} ${renderDarkTheme()}`}
		>
